...
class DataRowCollectionInsertRowSpec extends FlatSpec with Matchers {
  ...
  "DataRowCollection.insert" should "allow a valid row to be inserted at the specified row index" in {
  ...
  it should "allow a valid row to be inserted into the table using implicit value converters" in {
    ...
    val newTable = originalTable.rows.insert(3, 100, "TestVal", true)
  ...
  it should "fail to insert a row when the number of values is less than the number of columns" in {
    val originalTable = createTestTable
